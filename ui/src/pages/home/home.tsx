...
import { Grid, Row, Column, Tile, Tag } from "carbon-components-react";
...
export const Home = () => {
  ...
  const [cardListDetails, setCardListDetails] = useState<CardProps[]>([]);
  const [test, setTest] = useState(false);

  useEffect(() => {
    fetchDetails(appliedNameFilter);
  }, [appliedNameFilter]);

  const fetchDetails = async (appliedNameFilters) => {
    const companyList = await getFilterdCardData(appliedNameFilters);
    setCardListDetails(companyList);
  };
      ...
      <Grid style={{ padding: 0 }}>
        <Row className="PageContainer">
          <Column lg={5} style={{ paddingRight: 0 }}>
            <SideBar
              appliedFilter={appliedNameFilter}
              onChange={toggleFilter}
            />
          </Column>
          <Column lg={11} style={{ padding: 0 }}>
            <div className="TileContainer">
              {appliedNameFilter.length > 0 && (
                <Row>
                  {appliedNameFilter.map((filterName, index) => (
                    <Tag
                      className="Tag"
                      key={index}
                      type="gray"
